...
if (isset($_POST['reg_serPro'])) {
  ...
  $service_fee = mysqli_real_escape_string($db,$_POST['service_fee']);
  ...
  $user_check_query = "SELECT * FROM sp_list WHERE username='$username' OR email='$email' OR contact='$contact'LIMIT 1";
  ...
  $contact_check = $db->query("SELECT * FROM sp_list WHERE contact='$contact' LIMIT 1");
  ...
  if (count($errors) == 0) {
    ...
    $password = password_hash($password_1, PASSWORD_BCRYPT);//encrypt the password before saving in the database
  	...
  	$query = "INSERT INTO sp_list (firstname, lastname, username, email, password, contact, photo) 
...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    $query = "SELECT * FROM sp_list WHERE username='$username'";
